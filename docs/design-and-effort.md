...
| Public reads | Core block/transaction/outcome, unshielded and selected contract data; retained state needed to serve those reads and B | Complete contract analytics, advanced DUST snapshots/deltas, full wallet tree-sync interfaces and bridge analytics; staking/pool enrichment belongs to optional D |
...
| Core transaction/unshielded/selected-contract projections | 5–8 | — | — |
...
- **Node/matching fit:** a supported finalized node fixture yields versioned transaction offers and the expected positive/negative WASM viewing-key matches, including fallible offers. No official indexer supplies production inputs. Durable observation publication and retry preserve identities and declared source coverage. Missing payloads or required exports must be explicit before accepting the alpha estimate.
